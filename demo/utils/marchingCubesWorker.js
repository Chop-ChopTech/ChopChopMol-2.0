/**
 * Web Worker for Marching Cubes Isosurface Generation
 *
 * Runs the computationally intensive marching cubes algorithm in a separate thread
 * to keep the UI responsive during orbital visualization.
 *
 * Usage:
 *   const worker = new Worker('utils/marchingCubesWorker.js');
 *   worker.postMessage({ volumeData, gridInfo, isoValue, showPositive, showNegative });
 *   worker.onmessage = (e) => { const { vertices, normals } = e.data; };
 */
...
// Triangle table (compressed for worker size)
const TRI_TABLE = [
    ...
    [4,7,8,9,0,11,9,11,10,11,0,3,-1], [4,7,11,4,11,9,9,11,10,-1], [9,5,4,-1],
    [9,5,4,0,8,3,-1], [0,5,4,1,5,0,-1], [8,5,4,8,3,5,3,1,5,-1], [1,2,10,9,5,4,-1],
    ...
    [2,7,6,2,3,7,0,1,9,-1], [1,6,2,1,8,6,1,9,8,8,7,6,-1], [10,7,6,10,1,7,1,3,7,-1],
    ...
    [4,9,1,4,1,7,7,1,3,-1], [4,9,1,4,1,7,0,8,1,8,7,1,-1], [4,0,3,7,4,3,-1],
    [4,8,7,-1], [9,10,8,10,11,8,-1], [3,0,9,3,9,11,11,9,10,-1],
...
/**
 * Process a single isosurface (positive or negative phase).
 */
function processIsosurface(volumeData, gridInfo, iso, flipNormals) {
    ...
    const interpolate = (p1, p2, v1, v2, n1, n2) => {
    ...
    for (let ix = 0; ix < nx - 1; ix++) {
        for (let iy = 0; iy < ny - 1; iy++) {
            for (let iz = 0; iz < nz - 1; iz++) {
                // Get 8 corner values
                const vals = [
                    getValue(ix, iy, iz), getValue(ix + 1, iy, iz),
                    getValue(ix + 1, iy + 1, iz), getValue(ix, iy + 1, iz),
                    getValue(ix, iy, iz + 1), getValue(ix + 1, iy, iz + 1),
                    getValue(ix + 1, iy + 1, iz + 1), getValue(ix, iy + 1, iz + 1)
                ];
                ...
                const positions = [
                    getPosition(ix, iy, iz), getPosition(ix + 1, iy, iz),
                    getPosition(ix + 1, iy + 1, iz), getPosition(ix, iy + 1, iz),
                    getPosition(ix, iy, iz + 1), getPosition(ix + 1, iy, iz + 1),
                    getPosition(ix + 1, iy + 1, iz + 1), getPosition(ix, iy + 1, iz + 1)
                ];

                const grads = [
                    getGradient(ix, iy, iz), getGradient(ix + 1, iy, iz),
                    getGradient(ix + 1, iy + 1, iz), getGradient(ix, iy + 1, iz),
                    getGradient(ix, iy, iz + 1), getGradient(ix + 1, iy, iz + 1),
                    getGradient(ix + 1, iy + 1, iz + 1), getGradient(ix, iy + 1, iz + 1)
                ];
                ...
                for (let e = 0; e < 12; e++) {
                    if (edges & (1 << e)) {
                        const [v1i, v2i] = EDGE_VERTICES[e];
                        edgeData[e] = interpolate(
...
self.onmessage = function(e) {
    const { volumeData, gridInfo, isoValue, showPositive = true, showNegative = true } = e.data;
    ...
    if (showPositive) {
        positive = processIsosurface(volumeData, gridInfo, isoValue, false);
    }

    if (showNegative) {
        negative = processIsosurface(volumeData, gridInfo, -isoValue, true);
    }
    ...
    self.postMessage({
        positive: positive ? {
            vertices: new Float32Array(positive.vertices),
            normals: new Float32Array(positive.normals)
        } : null,
        negative: negative ? {
            vertices: new Float32Array(negative.vertices),
            normals: new Float32Array(negative.normals)
        } : null,
        elapsed
    }, positive && negative ? [
        positive.vertices.buffer || new Float32Array(positive.vertices).buffer,
        positive.normals.buffer || new Float32Array(positive.normals).buffer,
        negative.vertices.buffer || new Float32Array(negative.vertices).buffer,
        negative.normals.buffer || new Float32Array(negative.normals).buffer
    ] : []);
};
